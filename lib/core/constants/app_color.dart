import 'package:flutter/material.dart';

class AppColors {
  static Color pagescolor = Color(0xFFFCFCFD);
  static Color primaryColor = Color(0xFF7F56D9);
  static Color secondaryColor = Color(0xFF6941C6);
  static Color titleColor = Color(0xFF1F2A37);
  static Color whiteColor = Color(0xFFFFFFFF);
  static Color locationColor = Color(0xFF9DA4AE);
  static const Color dotColor = Color(0xFFD2D6DB);
  static Color redColor = Color(0xFFF04438);
  static Color starColor = Color(0xFFFDB022);
  static Color svgBackground = Color(0XFFF9F5FF);
  static Color borderColor = Color(0xFFE5E7EB);
  static Color redHeartColor = Color(0xFFF97066);
  static Color allBorderColor = Color(0xFFD9D9D9);
  static Color ratingColor = Color(0xFFFFFAEB);
  static const Color neutralGray600 = Color(0xFF4D5761);
  static Color inputBorderColor = const Color(0xFFD0D5DD);
  static Color inputHintColor = const Color(0xFF98A2B3);
   static Color fieldlabel=Color(0xFF344054);
    
}
